...
for song in songs:
    ...
    try:
        if items[0]['name'].lower() in title.lower() or all(new_artist['name'].lower() in artist.lower() for new_artist in items[0]['artists']):
        ...
        try:
            for item in items:
                ...
                for new_artist in new_song_artists:
                    ...
                    #and (item['name'].lower().split()[0] in title.lower()):
